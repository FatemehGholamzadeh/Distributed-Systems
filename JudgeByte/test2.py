...
def judgement():
    HOST_ROOT = os.open("/", os.O_RDONLY)
    global SCORE_UPDATED
    print(os.environ.get('containerID'))
    cID = os.environ.get('containerID')
    if 'leaderboard' in sys.argv:
        updateLeaderBoard()
        sys.exit(0)
    with open("/judgebyte/turingoj/private_media/submissions/cache/" + str(cID) + ".txt") as file_in:
        content = file_in.read()
        cache_submissions = []
        print(content.split("$"))
        for line in content.split("$"):
            if line !='':
                cache_submissions.append(SubmissionCache.objects.get(id=int(line)))
    print(cache_submissions)
